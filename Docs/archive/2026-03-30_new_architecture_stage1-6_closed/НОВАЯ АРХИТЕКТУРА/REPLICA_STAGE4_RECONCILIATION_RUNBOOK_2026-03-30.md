...
6. `scripts/stage4/Repair-HistoryOrderNumbersFromApi.ps1`  
   Точечный recovery: backfill пустых `Id/OrderNumber` в `history.json` из текущего API snapshot (с auto-backup).
7. `scripts/stage4/Get-ReconciliationOpsStatus.ps1`  
   One-shot операторский статус: API `/live` + Scheduler + latest report + latest journal entry.
...
```powershell
powershell -ExecutionPolicy Bypass -File scripts/stage4/Get-ReconciliationOpsStatus.ps1
```

Если нужен non-zero exit code при рисках:

```powershell
powershell -ExecutionPolicy Bypass -File scripts/stage4/Get-ReconciliationOpsStatus.ps1 -FailOnRisk
```
...
## Targeted Recovery (OrderNumber Gap)

Использовать, когда reconciliation показывает `payload_mismatch` по полю `OrderNumber`, а в `history.json` для того же `internal_id` значение пустое.

Dry-run:

```powershell
powershell -ExecutionPolicy Bypass -File scripts/stage4/Repair-HistoryOrderNumbersFromApi.ps1 -DryRun
```

Apply:

```powershell
powershell -ExecutionPolicy Bypass -File scripts/stage4/Repair-HistoryOrderNumbersFromApi.ps1
```

После apply:
1. Скрипт создаёт backup `history.json.bak-<timestamp>`.
2. Выполнить `Run-ReconciliationLive.ps1` повторно.
3. Убедиться, что `is_zero_diff=true`.
