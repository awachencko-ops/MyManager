...
8. Дополнительная регрессия после сервисного выноса LAN run/stop orchestration (2026-03-20):
   - `dotnet test Replica.sln` -> `60/60 PASS` (`35/35 Verify`, `25/25 UiSmoke`);
   - `REPLICA_RUN_PG_INTEGRATION=1 dotnet test tests/Replica.VerifyTests/Replica.VerifyTests.csproj` -> `35/35 PASS`.
...
5. Дополнительный baseline после сервисного выноса LAN run/stop orchestration (2026-03-20): `dotnet test Replica.sln` -> `60/60 PASS`.
