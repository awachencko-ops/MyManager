...
>
> Актуализация на 2026-03-26 (risk-burndown, срез 46): `IOrderApplicationService` расширен outcome-ориентированным LAN write/application boundary (`ApplyLanOrderWriteResult`, `SyncLanItemReorderForOrdersAsync`, `SyncLanOrderItemUpsertAsync`, `SyncLanOrderItemDeleteAsync`), а `OrdersWorkspaceForm` убрал локальные write/sync orchestration helper-методы и перешёл на применение outcome (logs/snapshot/status/dialog). Добавлены verify-тесты на write/sync outcome сценарии.
>
> Актуализация на 2026-03-27 (risk-burndown, срез 47): `OrderGridMutationUiPlan` вынесен в application boundary; `PersistGridChanges(...)` и delete/file-ops refresh policy теперь применяют единый outcome-план вместо локального save/rebuild branching. Последний UI-orchestration хвост god-object'а закрыт на текущем cutover.
>
...
>
> Актуализация на 2026-03-23 (risk-burndown, срез 41): LAN write-boundary расширен на item delete (`DELETE /api/orders/{id}/items/{itemId}`) через `LanOrderWriteApiGateway`/`LanOrderWriteCommandService`; `OrdersWorkspaceForm` переведён на API-first item delete path для `RemoveSelectedOrderItems` (в LAN режиме), а в file remove/move ветках (`RemoveFileFromItem`, `drag-move clear-source`) добавлена синхронизация delete/upsert через API boundary; добавлены unit-тесты gateway/command-service и PostgreSQL integration тест `PostgreSqlIntegration_EfCoreStore_DeleteItem_ReindexesAndAppendsEvent`, подтверждены build + full test + PostgreSQL integration regression.
...
- В `IOrderApplicationService` добавлены history/folder orchestration операции; форма больше не обращается напрямую к history coordinator/maintenance/folder-resolution сервисам.
...
35. Итерация 35 (2026-03-23, адресная): закрыт следующий write-boundary срез для LAN item delete (`remove/move` сценарии).
   - Что сделано: API расширен `DeleteOrderItemRequest` + endpoint `DELETE /api/orders/{id}/items/{itemId}` и store-реализациями `TryDeleteItem` (`EfCoreLanOrderStore`, `PostgreSqlLanOrderStore`, `InMemoryLanOrderStore`) с optimistic concurrency (`ExpectedOrderVersion`, `ExpectedItemVersion`), reindex sequence и событием `delete-item`; client boundary расширен `DeleteOrderItemAsync/TryDeleteItemAsync/TryDeleteOrderItemViaLanApiAsync`; `OrdersWorkspaceForm.RemoveSelectedOrderItems` в LAN-режиме переключён на API-first удаление item, а `StageFileOps/GridInteractions` для remove/move веток синхронизируют delete/upsert через LAN API.
   - Эффект: закрыт заявленный scope write-command boundary для `create/update/items/reorder/status` + `item delete`; удаление item теперь проходит через server source-of-truth и не опирается на локальный reorder-only компенсационный путь.
36. Итерация 36 (2026-03-23, адресная): закрыт следующий write-boundary срез для LAN order delete (single/batch).
   - Что сделано: API расширен `DeleteOrderRequest` + endpoint `DELETE /api/orders/{id}` и store-реализациями `TryDeleteOrder` (`EfCoreLanOrderStore`, `PostgreSqlLanOrderStore`, `InMemoryLanOrderStore`) с optimistic concurrency и событием `delete-order`; `OrdersController` применяет `Idempotency-Key` для delete-order; client boundary расширен `DeleteOrderAsync/TryDeleteOrderAsync/TryDeleteOrderViaLanApiAsync`; `OrdersWorkspaceForm.RemoveSelectedOrder` в LAN-режиме переведён на API-first path (single/batch delete) с optional disk cleanup и snapshot refresh.
   - Эффект: write-command boundary закрыт и для удаления заказов; в LAN PostgreSQL режиме order delete теперь фиксируется server-side как source-of-truth и подтверждён автотестами.
37. Итерация 37 (2026-03-26, адресная): закрыт следующий presenter-only срез LAN write outcomes.
   - Что сделано: в `IOrderApplicationService/OrderApplicationService` добавлены outcome-методы `ApplyLanOrderWriteResult`, `SyncLanItemReorderForOrdersAsync`, `SyncLanOrderItemUpsertAsync`, `SyncLanOrderItemDeleteAsync`; `OrdersWorkspaceForm` удалил локальные helper-ветки `TryApplyLanOrderUpdateResult/ShowLanWriteError/TrySyncLan*` в пользу application boundary + единых outcome.
   - Эффект: UI-слой дополнительно упрощён до применения outcome (status/logs/dialog/snapshot), снижена дублируемость LAN write-веток и риск расхождения поведения между create/edit/item workflows.
...
2. **Статусные переходы и аудит «мимо транзакций»** — `PARTIAL`: status policy вынесена, `order_events` есть, server-side command handling и full write-idempotency закрыты в целевом LAN scope (`create/update/items/reorder/status` + `item delete` + `order delete` + `run/stop`); следующий шаг — унификация audit/observability.
3. **God Object (MainForm/OrdersWorkspaceForm как бизнес-оркестратор)** — `DONE (current cutover)`: вынесены history/run-state/status-transition/run-execution/delete-workflow/run-stop-preflight/create-edit/item-mutation/item-delete-command/order-delete-command/file-path-status-mutation/file-stage-command-planning/file-rename-remove-command/print-tiles-rename-sync/history-lifecycle-maintenance/folder-path-resolution/storage-version-sync/run-feedback/LAN write outcomes/status-persist fallback/post-file delete refresh policy; выполнен rename shell, модульный перенос UI-кода, старт DI/composition-root bootstrap (`OrdersWorkspaceCompositionRoot`) и введён unified orchestration boundary (`IOrderApplicationService`) с history/folder orchestration. Следующий фокус — perf/UX полировка и mediator/DI runtime cleanup.
