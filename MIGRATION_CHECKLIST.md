...
| Settings | Открытие/сохранение настроек | Legacy only | `Forms/SettingsDialogForm.cs` + `Models/AppSettings.cs` | - | Подвязать через MainForm |
...
1. Подключить из `MainForm` открытие `SettingsDialogForm`.
